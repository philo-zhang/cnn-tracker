...
def prediction(net, imgName, win, templates, weights):
    ...
    ratio = np.tile(ratio, (templates.shape[0],4))
    verticesNew = vertices - np.multiply(output, ratio)
    dWin = winParam((0,0),(0,0),0)
    ...
    print 'dSize:', dSize
    print 'dAngle:', dAngle
    for i in range(dCenter.shape[0]):
        dWin.center += dCenter[i]*weights[i]
        dWin.size += dSize[i]*weights[i]
        dWin.angle += dAngle[i]*weights[i]
    print 'dWin.size', dWin.size
    centerNew = (int(round(dWin.center[0]+win.center[0])), int(round(dWin.center[1]+win.center[1])))
    sizeNew = (int(round(dWin.size[0]*win.size[0])), int(round(dWin.size[0]*win.size[1])))
    angleNew = dWin.angle+win.angle
    return winParam(centerNew, sizeNew, angleNew) 
...
def judgment(win, rect, lost):
    ...
    if (minmaxX>maxminX and minmaxY>maxminY):
        ratio = (minmaxX-maxminX)*(minmaxY-maxminY)/(win.size[0]*win.size[1]+rect[2]*rect[3])
    else:
        ratio = 0
    if ratio<0.3:
...
def draw(videoWri, imgName, imgNum, win, lost):
    R = np.array([[math.cos(win.angle), -math.sin(win.angle)], [math.sin(win.angle), math.cos(win.angle)]])
        ...
        for imgNum in range(1, 1000):
            ...
            if imgNum == 1:
                ...
                for j in range(5):
                    ...
                    weights.append(0)
                weights[0] += 1
                videoWri = cv2.VideoWriter(seqName+'.avi', cv2.cv.CV_FOURCC('F','M','P','4'), 5, (img.shape[1], img.shape[0]))
