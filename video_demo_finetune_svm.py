...
def maxLikelihood(img, ipca, winsNew):
    ...
    ind = np.argmin(score)
    print 'score', score[ind]
    #print 'ind', ind
    return winsNew[ind], score[ind]


    
def prediction(net, imgName, win, templates, weights, ipca, svmClf):
    ...
    maxT = np.amax(np.absolute(templates))
    maxC = np.amax(np.absolute(candidates))
    templates = templates/maxT
    ...
    winMax, pcaScore = maxLikelihood(img, ipca, winsNew)
    if pcaScore > threshold:
        feat = np.array(out['fc5_p'])
        svmScore = svmClf.predict(feat)
        ind = np.argmax(svmScore)
        winMax = winsNew[ind]
...
def pos_neg_data(img, win, flag, dataSize):
    ratio = (win.size[0]/64.0, win.size[1]/64.0)
    candidates = []
    Y = np.zeros((dataSize,8), dtype = float)
    for i in range(dataSize):
        rand = np.random.uniform(-1,1,4)
        if flag==1:
            dCenter = np.multiply(rand[0:1], np.array(win.size)/5.0)
        else:
            if rand[0]==0:
                rand[0]==0.0001
            if rand[1]==0:
                rand[1]==0.0001
            dCenter = np.multiply(rand[0:1]+sign(rand[0:1])/3, np.array(win.size)*3/4)
        dAngle = rand[2]*5
        dSize = rand[3]*0.1
        centerNew = np.rint(win.center+dCenter).astype(int)
        angleNew = win.angle+dAngle
        sizeNew = np.rint(win.size*np.array(1+dSize)).astype(int)
        affMat = cv2.getRotationMatrix2D(tuple(centerNew), angleNew, 1.0)
        rotatedSrc = cv2.warpAffine(img, affMat, (img.shape[1], img.shape[0]))
        imcrop = cv2.getRectSubPix(rotatedSrc, tuple(sizeNew), tuple(centerNew))
        imcrop = cv2.resize(imcrop,(64,64))
        sine = math.sin(win.angle*PI/180)
        cosine = math.cos(win.angle*PI/180)
        scale = 1+dSize
        R = np.array([[scale*cosine, -scale*sine, dCenter[0]], [scale*sine, scale*cosine, dCenter[1]], [0, 0, 1]])
        ori = np.array([[-win.size[0]/2.0, win.size[0]/2.0, win.size[0]/2.0, -win.size[0]/2.0], [-win.size[1]/2.0, -win.size[1]/2.0, win.size[1]/2.0, win.size[1]/2.0], [1, 1, 1, 1]])
        new = np.dot(R, ori)
        dis = new-ori
        dis = np.divide(dis[0:2], np.reshape(ratio, (2,1)))
        dis = np.reshape(dis, (8,1)).transpose()
        Y[i] = dis
        candidate = np.zeros((3,64,64), dtype = float)
        for j in range(3):
            candidate[j] = imcrop[:,:,j]
        candidates.append(candidate)
    return candidates, Y
 
def finetune(img, win):
    imgObjTemp = img[win.center[1]-win.size[1]/2:win.center[1]+win.size[1]/2,win.center[0]-win.size[0]/2:win.center[0]+win.size[0]/2]
    imgObj = cv2.resize(imgObjTemp, (64,64))
    template = np.zeros((3,64,64), dtype = float)
    for j in range(3):
        template[j] = imgObj[:,:,j]
    templates = []
    print templates
    ...
    candidates, Y = pos_neg_data(img, win, 1, 200) 
    with h5py.File(dataPath + '/mean.h5','r') as f:
        Xmean = np.array(f['mean'])
    print 'xmean', Xmean[0,0,0:5,0:5]
    templates = np.array(templates)-Xmean
    candidates = np.array(candidates)-Xmean
    print 'templates', templates[0,0,0:5,0:5]
    print 'candidate', candidates[0,0,0:5,0:5]
    ...
    print Y[0:5]
    with h5py.File(dataPath + '/finetune/train_batch_0.h5', 'w') as w:
        w['data'] = X
        w['label'] = Y
    with open(dataPath + '/finetune/train.txt', 'w') as f:
        f.write(dataPath + '/finetune/train_batch_0.h5' +'\n')
    os.system('sh cnn-tracker/affine_finetune.sh')

def train_svm(svmClf, net, img, win):
    posCand = pos_neg_data(img, win, 1, 200)
    negCand = pos_neg_data(img, win, 1, 200)
    posCand = np.array(posCand)
    negCand = np.array(negCand)
    trainCand = np.concatenate((posCand, negCand), 1)
    label1 = np.ones(200, dtype = int)
    label2 = np.zeros(200, dtype = int)
    label = np.concatenate((label1, label2), 0)
    out = net.forward_all(data=trainCand)
    featPos = np.array(out['fc5'])
    featNeg = np.array(out['fc5_p'])
    X = np.concatenate((featPos, featNeg), 1)
    svmClf.fit(X, label)
    return svmClf
...
if __name__ == '__main__':
    if len(sys.argv) != 2:
        ...
        for imgNum in range(1, 2000):
            ...
            svmClf = train_svm(svmClf, net, img, win) 
        #videoWri.release()
